...
/**
 * 将两个option合并为一个新的option对象
 * Merge two option objects into a new one.
 * 这是一个在实例化和继承中都使用的核心工具
...
class StringNode {
    constructor(open, close, children, normalizationType) {
        this.isString = true;
        this.open = open;
        this.close = close;
        if (children) {
...
function renderSSRClass(staticClass, dynamic) {
    const res = renderClass$1(staticClass, dynamic);
...
class TemplateStream extends stream.Transform {
...
function normalizeFile(file) {
    ...
    const extension = path$2.extname(withoutQuery).slice(1);
    return {
        file,
        extension,
...
function createRenderer$1({ modules = [], directives = {}, isUnaryTag = () => false, template, inject, cache, shouldPreload, shouldPrefetch, clientManifest, serializer } = {}) {
...
const createBundleRenderer = createBundleRendererCreator(createRenderer);
